...
void setup()
{
  ...
  device.setName("theAirboard");
  device.setSoftwareVersion("1.0.0");
  device.enableSharedAvailability();
  device.enableLastWill();

  // configure sensor (optional)
  theAirboardTemperature.setIcon("mdi:thermometer");
  theAirboardTemperature.setName("Cooker temperature");

  theAirboardBattery.setIcon("mdi:battery");
  theAirboardBattery.setName("TheAirboard battery");

  mqtt.begin("emqx.home-assistant.localdomain");
...
//   }
  ...
  if (now - statusPreviousMillis >= STATUS_UPDATE_INTERVAL) {
    if (mqttClient.connected()) {
      ...
      theAirboardBattery.setValue(board.batteryChk());
      theAirboardTemperature.setValue((float)board.getTemp());
    }
  }
